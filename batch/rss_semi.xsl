...
INSERT INTO Seminars(Title, ShortDescription, Text, State,Place,EventStartDate,EventEndDate) VALUES('<xsl:value-of select="title"></xsl:value-of>', '<xsl:value-of select="subtitle"></xsl:value-of>', '<xsl:value-of select="annotation"></xsl:value-of>','1','Винзавод',GETDATE(),GETDATE());
