...
<% using (var form = new CustomEntityForm(Html, Model) { Title = "Добавление семинара", BlockClass = "addMaterialsBlock", ListClass = "iForm", Action = "SaveAjax" }.Open())
   {
       ...
       using (var group = new CustomGroup(Html, form) { Title = "Принадлежность к циклу" }.Open())
       {
           %>
           <%
           new SeminarModule() { ViewName = "SingleCycleWidgetForEdit", Id = Model.REL_Id }.SingleWidget(Html, "SingleCycleWidgetForEdit");
       }
       ...
       using (var group = new CustomGroup(Html, form) { Title = "Семинар" }.Open())
       {
           using (var wrap = new CustomWrap(Html) { Wrap = "addArticle" }.Open())
           {
