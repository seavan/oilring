...
<div class="materialsOne">
    <div class="infoLine">
        <div class="date">
            Размещена
            <%=Model.PublicationDate.HasValue
                             ? Model.PublicationDate.Value.ToStringNormalDate()
                             : Model.CreationDate.ToStringNormalDate()%>
            <% if (Model.ModificationDate.HasValue)
{ %>
            (Последние изменения
            <%=Model.ModificationDate.Value.ToStringNormalDate() + " " +
                      Model.ModificationDate.Value.ToShortTimeString()%>)
            <% } %>
        </div>
        ...
        <div class="block6">
            ...
            <div class="commentsBlock">
                <h4>
                    Комментарии</h4>
                <%
       (new CommentModule() {REL_ObjectID = Model.Id, REL_ObjectType = Model.ObjectType}).ListWidget(Html).AddWidget(
           Html);%>
            </div>
            <!--/комменты-->
        </div>
...
{%>
                    <span>(Семинар еще не прошел)</span>
                    <% }%>
                </dd>
            </dl>
            <!--/-->
            ...
            <!--/-->
            <!---->
            <% (new RandomSeminarModule(Model)).ListRandom(Html); %>
            <!--/-->
            <!---->
