        ...
        public Dummy_SearchObjectController()
        {
            ...
            m_PreFilterList.RegisterSelector(
                new BasicFilter<IDatabaseEntity>(
                    ...
                    (p, src) =>
                    {
                        return new Dummy_SearchObjectService().SearchByTagId(p.SearchTagId);
                    },
                        (p) => { return p.SearchTagId > 0; },
                        (p) => { return new object[] { p.SearchTagId }; }
                    )
                    );
            ...
            m_PreFilterList.RegisterSelector(
                new BasicFilter<IDatabaseEntity>(
                    ...
                    (p, src) =>
                    {
                        return new Dummy_SearchObjectService().SearchByRubricId(p.SearchRubricId);
                    },
                        (p) => { return p.SearchRubricId > 0; },
                        (p) => { return new object[] { p.SearchRubricId }; }
                    )
                    );
